<?php
/**  ecomextra Version 1.0
 $Id: cls_fast_template.php 01/24/2013 ecomextra  $
 * This version ported for use with ecomextra. It is not intended as a rerelease
 * or update to any previous versions.
 * Adapted from code available @ GraFX Software Solutions 
 * webmaster@grafxsoftware.com
 * Most changes are due PHP 5.3.0, the regex extensions are deprecated in favor 
 * of the PCRE extensions. These changes are not notated in the code.
 * 
...
if (! class_exists ( 'FastTemplate' )) {
	...
	class FastTemplate {
		 ...
		 *         Unresolved vars will display red in template.
		 */
			...
			if (preg_match ( "/\{[A-Z0-9_]+\}/", $Line, $unknown)) {
				...
				if (! (empty ( $UnkVar ))) {
					...
					if ($this->STRICT_DEBUG) {
						$this->WARNINGS  = "[FastTemplate] Warning: no value found for variable: $UnkVar[0] \n";
					}

					if ($this->STRICT) {
						@error_log ( "[FastTemplate] Warning: no value found for variable: $UnkVar[0] ", 0 );
					}
				}
			}
		} // end show_unknowns()
		...
		FUNCTION parse_template($template, $ft_array) {
			...
			if (! $this->STRICT || ($this->STRICT && ! $this->STRICT_DEBUG)) { //Fixed error ^^ // by Voituk Vadim
				...
				$template = preg_replace ( "/\{([A-Za-z0-9_\.]+)\}/", "", $template ); // by Voituk Vadim correct using preg_replace insted ereg_replace, small addition a-z, thanx to Gabe Alack, the regex used in the line to remove unset variables only checked for all caps, while lowercase variables are allowed by the class. (1.6.2)
				...
				// Changed for ecomextra
				if (preg_match_all ( "/\{([a-zA-Z_][a-zA-Z0-9_]+)\}/", $template,$matches )) {
				 if ($this->STRICT_DEBUG){
				FOR($i = 0; $i < count ( $matches [0] ); ++ $i) {
				$new = $matches[0] [$i];
				$template = str_replace ( $matches [0] [$i], "<font style='color:red'>" . $new . "</font>" , $template  );
				  }
				}
							$unknown = explode ( "\n", $template );
		...
		FUNCTION parse($ReturnVar, $FileTags) {
			...
			$FileTagsOrdered=array_reverse($FileTags); //added to enable listing multiple templates in the order to be displayed. Top to bottom ecomextra.
				unset ( $this->$ReturnVar ); // Clear any previous data
			//	array_reverse($FileTags);
				WHILE ( list ( $key, $val ) = each ( $FileTagsOrdered ) ) {
		 ...
		 * @author   - ecomextra
		 * @type     - public
		 * @desc     - Prints parsed templates.
		 *	     - Passes template names to FastPrint individually from an array.
		 *				<code>
		 * 					$tpl->FastPrintArray(array("MAIN","FOOTER")); 
		 *				</code>
		 * @param    - $array the template names
		 * @param    - $return if TRUE template is returned.
		 * @return   - template string
		 * @vers     - 1.1
		 * @Mod by   - 
		 * @Mod vers -
		 */
		FUNCTION FastPrintArray($array){
			foreach($array as $key =>$value){
			$this->FastPrint($value);
			}
		} //end FastPrintArray
		 ...
		 * @vers     - 1.0
		 * @Mod by   - ecomextra
		 * @Mod vers - 0.2 ensures cached files have correct $cPath and
		 *             Languages id.
		 *             
		 **/
		...
		global $languages_id;
			...
			$fname .= "m=" . $_GET["manufacturers_id"];
			}
			if(!empty($_GET["cPath"] )) {
			$fname .= "c=" . $_GET["cPath"];
			}
			if(!empty($_GET["products_id"] )) {
			$fname .= "pid=" . $_GET["cPath"];
			}

			$fname .=  "_lng_" . $languages_id;
